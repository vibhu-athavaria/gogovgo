...
class ShareReviewURLModal extends Component {
	...
	componentWillReceiveProps(nextProps) {
		const reviewURL = window.location.href.replace('#', '') + '/reviews/' + encrypt(nextProps.reviewId);
		this.setState({value: reviewURL})
	}

	render() {
		const {reviewId, onHide, ...rest} = this.props;
			...
			<Modal {...rest} dialogClassName="custom-modal">
				...
				<Modal.Body>
					<div className="text-center">
						...
						<div className="your-review-has-been">
							Your rating has been published. Copy the link below to share it with others.
						</div>
					</div>
				...
				<Modal.Footer>
					<div className="form-group text-center margin_abajo_medium">
						<button type="button" className="btn btn-modal btn-link" onClick={onClose}>Close</button>
					</div>
				</Modal.Footer>
			</Modal>
		);
