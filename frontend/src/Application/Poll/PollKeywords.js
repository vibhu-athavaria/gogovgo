...
class PollKeywords extends Component {
	...
	render() {
		// Local variables
		const { approved, onHide, politicianId, politicianName, location, suggestedTags, ...rest } = this.props;
		...
		const reasonInputPlaceholder = this.state.suggestions.slice(0, 3).map(tag => {
			return tag.name
		}).join(', ');
